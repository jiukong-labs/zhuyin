...
After installation, the registration helper must report the source ID `tw.org.cloudgate.jiukong.inputmethod.zhuyin`, the expected type, and an accepted enable request. It does not select the source. The final UI check is **System Settings > Keyboard > Text Input > Edit… > 久空輸入法**; if approval is pending, use **+ > Traditional Chinese > 久空輸入法**. If a system-session cache prevents it from appearing, sign out and back in once.
