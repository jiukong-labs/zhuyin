...
`main.swift` validates the bundle configuration, creates exactly one `IMKServer`, and starts the AppKit run loop. Its `JiukongZhuyin_1_Connection` service name uses the InputMethodKit form recognized by macOS for cold launches. The bundle is an `LSUIElement` agent, so it can own a nonactivating candidate panel without appearing in the Dock. InputMethodKit creates one `InputController` for each client input session.
...
A toggle first uses the existing idempotent composition finalization path, then changes language according to the persisted `ShiftSwitchStyle`. The default `inputSource` style calls `TISSelectInputSource` for the other Jiukong mode. This keeps the process-wide language state and the macOS input-menu icon synchronized, while allowing macOS to present its native transient input-source indicator. The `withinInputMethod` style changes only `LanguageModeController` and leaves the selected source alone, so no client observes a source change. It does so only while the Chinese mode is selected: with the English mode selected, which macOS also forces for a password field, a toggle selects the Chinese mode as the default style does, so macOS can refuse it for a field that accepts ASCII input sources only. Otherwise Bopomofo would compose into that field. After an in-method toggle, `isInternallyManaged` keeps a client activation from resetting the language to the still-selected mode, and `LanguageModeHUD` gives feedback when the cursor indicator is off. In both styles a selected-source notification for a Jiukong mode is the user's latest explicit choice and is adopted, and the cursor indicator shows `LanguageModeController`'s language except on that notification. Switching back to the `inputSource` style selects the mode Jiukong is actually in. Chinese mode continues through candidate and Bopomofo handling. English mode returns key events unchanged, allowing the client and selected macOS keyboard layout to own characters, capitalization, dead keys, repeats, and shortcuts. A new process defaults to Chinese until a client activation reports the selected mode.

While a client has Jiukong active, `ClientDeliveryFallback` polls the session keyboard state every 15 ms. It switches language for a standalone Shift tap the client never delivered, arbitrating with the event path so one tap switches once. After a switch to Chinese, `SilentClientDetector` watches for plain keys that never reach the input method. After a switch that selected the Chinese source, a key unseen for 80 ms makes the controller select English and then Chinese again, holding keys typed meanwhile for replay in Chinese; after a switch made inside Jiukong no source change caused the silence, so the watch only records it.
...
`UserLearningStore` owns a versioned SQLite database under the user Application Support directory. It validates `application_id`, schema version, columns, and regular-file identity before use; enables full-mutex access, a one-second busy timeout, WAL, normal synchronous writes, and foreign keys; and keeps the directory/database at `0700`/`0600`. An atomic UPSERT saturates counts at `Int64.max`, keeps the newest timestamp, and preserves an existing pin. A separate `suppressed_phrases` table records which built-in dictionary phrases the user removed, keyed by the same length-prefixed reading key user phrases use; candidate lookup drops those texts from the dictionary's own phrase results only. Because the tombstone lives in the user's database rather than in the bundled dictionary, a dictionary shipped with a later update cannot restore a removed phrase. `UserLearningService` serializes process-wide access and logs only generic failures without characters or readings. If storage fails, candidate lookup continues with the immutable built-in default ranking.

`PhraseSharePack` is the separate document one person hands to another. It carries only phrase identities — text, ordered readings, and the reading/punctuation pattern — for the user's own phrases and the built-in phrases they removed, so a shared list discloses no selection counts, timestamps, or pins. The bundled dictionary is never copied into a pack: both Macs ship the identical lexicon, so a pack reproduces the sender's word list without redistributing bundled data. Decoding is total and revalidates every row through `UserPhraseValidator`, because a pack is untrusted input from another person. Import converts the pack into a `UserDataArchive` with zero counts and no pins and reuses the existing merge, so an imported word can never lower a count or clear a pin the recipient set, and importing twice is idempotent. Applying the sender's removals is a separate opt-out at import time.
...
Automatic learning gates only implicit recording. Existing counts and pins still rank candidates while it is off, and explicit Shift phrase creation still works, because the user asked for that phrase directly. `UserLearningStore` clears character data, phrase data, removed built-in phrases, or all of them inside one immediate transaction that revalidates the schema, so the database stays usable without reopening and a failure cannot half-clear the data.

`SettingsWindowController` owns the single settings window for every client session. Because the bundle is an agent, it activates the process explicitly before ordering the window front and deactivates again on close. `InputController.menu()` contributes the input-menu item and finalizes any active composition before the window opens. Six tabs separate general settings, the cursor indicator, the phrase list, the removed built-in phrase list, the character list, and data transfer with iCloud status; each `UserDataListController` re-reads its snapshot after every edit so a list can never act on an entry another window already deleted.
...
Two rules in it are load-bearing. The system's own Zhuyin input method composes identical Bopomofo from identical keys, so every run first requires Option-A and Option-Z to produce Jiukong's literal `az`; a system keyboard layout produces different Option characters, and this behavioral check cannot mistake the cursor indicator for a candidate panel. And because a running application keeps the input source it already adopted, the harness selects the source first and then launches a new client instance rather than trying to convert an existing one.
