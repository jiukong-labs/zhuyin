...
The schema indexes `pronunciation → characters`, `character → pronunciations`, and exact `pronunciation sequence → phrases`. Each character-reading row also stores how many exact occurrences it has in Jiukong's manually authored phrase TSV. Government-sourced idiom and revised-dictionary entries are excluded from this value. A reviewed first-party reading reuses that character's pinned CNS source position, and first-party phrase ties preserve repository file order. Unicode Plane 15 private-use mappings are deliberately excluded because they have no portable character identity or glyph without additional proprietary conventions.
...
Each character candidate's `baseFrequency` is `2 − usage_tier + count / (count + 1)`: MOE's 常用/次常用/其他 three-tier classification, per (character, reading) once a manually reviewed heteronym override is applied on top of the character-wide MOE tier, plus the exact first-party phrase-attestation count described above. The fractional attestation bonus is always below one, so it orders candidates within a tier without allowing a lower tier to cross a higher one; equal counts fall back to `source_order`. Because MOE classifies a whole character rather than one of its readings, an override is still the mechanism that ranks an unattested rare reading below common readings of other common characters (e.g. 疋/ㄕㄨ, 食/ㄧˋ, and 射/ㄧˋ — see `Data/JiukongHeteronyms/README.md`). This is a project-owned ranking signal, not a corpus-derived frequency count. Exact phrases keep their own neutral zero base score plus an independent tier bonus.
